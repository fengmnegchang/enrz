...
public class MobileMainService extends CommonService {
	public static final String TAG = MobileMainService.class.getSimpleName();
					...
					for (int i = 0; i < moduleElements.size(); i++) {
						...
						try {
							...
							if (titleLeftElement != null) {
								...
								//ul.gn-sub
								try {
									//<li><a target="_blank" href="http://enrz.com/fashion/mix">搭配栏目</a></li>
									Elements liElements = moduleElements.get(i).select("li");
									if(liElements!=null && liElements.size()>0){
										List<GnSubBean> gnsublist = new ArrayList<GnSubBean>();
										GnSubBean subbean;
										for(int j=0;j<liElements.size();j++){
											subbean = new GnSubBean();
											try {
												Element aElement = liElements.get(j).select("a").first();
												String titles = aElement.text();
												String hrefs = aElement.attr("href");
												Log.i(TAG, "i==" + i + ";titles==" + titles + ";hrefs==" + hrefs);
												subbean.setHref(hrefs);
												subbean.setTarget(titles);
											} catch (Exception e) {
												e.printStackTrace();
											}
											gnsublist.add(subbean);
										}
										gbean.setGnsub(gnsublist);
									}
								} catch (Exception e) {
									e.printStackTrace();
								}
